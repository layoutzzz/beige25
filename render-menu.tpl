...
{if $place eq 'mainmenu' || $place eq 'inventory'}
	{if $place eq 'mainmenu'}
		<a href="#top-menu" class="menu-link">Menu</a>
		<nav id="top-menu" class="clearfix" role="navigation"><ul class="menu clearfix">{/if}
		{foreach from=$block.contents item=menu}
			...
			{/if}
		{/foreach}
	{if $place eq 'mainmenu'}</ul></nav>{/if}
