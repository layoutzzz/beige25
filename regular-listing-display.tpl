<tr>
	...
	<div class="url{if $config.thumbshot} display-thumb{/if}">{$listing.url|truncate:40:"...":true}</div>
	...
	<div class="stat">({$lang.clicks}: {$listing.clicks};
		...
		{if $config.broken_listings_report && not ($esynAccountInfo.id eq $listing.account_id)}
			<a href="#" class="esynactions|report-listing|{$listing.id}" rel="nofollow">{print_img fl="report_16.png" full=true alt=$lang.report_broken_listing title=$lang.report_broken_listing}</a>
		{/if}
		...
		{if $esynAccountInfo}
			{if $esynAccountInfo.id neq $listing.account_id}
				...
				{if isset($listing.favorite) && !$listing.favorite}
					<a href="#" class="esynactions|add-favorite|{$listing.id}|{$esynAccountInfo.id}" rel="nofollow">{print_img fl="favorites-add_16.png" full=true alt=$lang.add_to_favorites title=$lang.add_to_favorites}</a>
				{else}
					<a href="#" class="esynactions|remove-favorite|{$listing.id}|{$esynAccountInfo.id}" rel="nofollow">{print_img fl="favorites-remove_16.png" full=true alt=$lang.remove_from_favorites title=$lang.remove_from_favorites}</a>
				{/if}
				</span>
			{else}
				<a href="#" class="esynactions|delete-listing|{$listing.id}">{print_img fl="delete.png" full=true alt=$lang.delete_listing title=$lang.delete_listing}</a>
			{/if}
		{/if}

		{if isset($listing.crossed) && $listing.crossed eq '0' && $esynAccountInfo.id eq $listing.account_id}
			<a href="#" class="esynactions|move|{$listing.id}|{$listing.category_id}">{print_img fl="move_16.png" full=true alt=$lang.move_listing title=$lang.move_listing}</a><br />
